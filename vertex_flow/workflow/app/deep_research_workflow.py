...
请提供完整的研究分析框架，为后续的自动化分析流程提供指导。
    ...
    def _get_information_collection_system_prompt(self) -> str:
        ...
        current_date = datetime.now().strftime("%Y年%m月%d日")
        return f"""
你是一位专业的信息分析专家。你需要根据前面制定的研究分析框架，主动收集、整理和初步分析相关信息。

**重要信息：今天是{current_date}**

你可以使用web_search工具来搜索最新的相关信息。请根据研究分析框架中的搜索策略和关键信息点进行多次搜索，获取全面的信息并进行初步整理分析。在搜索时，请特别关注时效性，优先获取最新的信息和动态。

请按照以下结构进行信息收集和初步分析：
1. 基础信息收集：收集主题的基本概念、定义和背景，并进行概念澄清
2. 历史发展脉络：梳理主题的发展历程和重要里程碑，识别发展规律
3. 现状全景分析：分析当前的发展状况、主要特点和关键参与者
4. 技术深度解析：深入了解相关技术原理、实现方式和技术趋势
5. 市场生态分析：分析市场规模、竞争格局、商业模式和价值链
6. 典型案例研究：收集和分析典型案例、成功实践和失败教训
7. 专家观点汇总：整理行业专家、学者和意见领袖的观点和预测
8. 最新动态追踪：通过web搜索获取最新的相关新闻、研究成果和发展动态（特别关注{current_date}前后的最新信息）

在收集信息时，请主动使用web_search工具搜索相关关键词，确保信息的时效性和全面性。对收集到的信息进行初步的分类、整理和关联分析。
...
请按照以下结构进行深度分析：
1. 趋势分析：识别发展趋势和变化模式，特别关注当前时间点的趋势特征
2. 关联分析：分析不同因素之间的关联关系
3. 优势劣势：分析主题的优势、劣势、机会和威胁（SWOT分析）
4. 技术评估：评估技术成熟度和发展潜力
5. 风险评估：识别潜在风险和挑战
6. 影响分析：分析对相关行业和社会的影响
7. 创新机会：识别创新点和发展机会
8. 时间敏感性：分析时间因素对主题发展的影响
9. 深层洞察：提供独特的分析视角和见解

请提供深入、有价值的分析报告和洞察，特别关注当前时间背景下的分析价值。
        """.strip()
