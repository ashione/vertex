...
class ErrorHandler:
    ...
    def format_api_error(
        service_name: str, error: Exception, response: Optional[requests.Response] = None
    ) -> Dict[str, Any]:
        """
        格式化API错误信息
        ...
        Returns:
            格式化的错误信息字典
        """
        error_info = {"error": True, "service": service_name, "type": type(error).__name__, "message": str(error)}
    ...
    def format_simple_error(service_name: str, message: str) -> Dict[str, Any]:
        ...
        Returns:
            格式化的错误信息
        """
        return {"error": True, "service": service_name, "message": message, "user_message": message}
