        ...
        if "error" in spot_positions:
            ...
            if spot_data:
                ...
                for position in spot_data:
                    ...
                    print(f"  {currency}: Available={available:.8f}, "
                          f"Frozen={frozen:.8f}, Total={balance:.8f}")
            else:
        ...
        if "error" in futures_positions:
            ...
            if futures_data:
                ...
                for position in futures_data:
                    ...
                    print(f"  {symbol}: Side={side}, Size={size:.8f}, "
                          f"Entry=${entry_price:.4f}, PnL=${pnl_str}")
            else:
...
def futures_metrics_example():
    ...
    if all_futures_positions:
        ...
        portfolio_metrics = PositionMetrics.calculate_portfolio_metrics(
            all_futures_positions, total_balance
        )
