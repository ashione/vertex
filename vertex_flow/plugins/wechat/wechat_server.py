        ...
        if msg_type == "text":
            ...
            if ENABLE_ASYNC_PROCESSING:
                try:
                    ...
                    # 提交后台任务
                    task_id = await async_proc.submit_task(user_openid=from_user, message_content=content)
                    ...
                    while time.time() - start_time < wait_timeout:
                        ...
                        if task_status.get("status") == "completed":
                            ...
                            if result:
                                ...
                                logger.info(
                                    f"任务快速完成，直接返回结果: {task_id}, 耗时: {time.time() - start_time:.2f}秒"
                                )
                                return PlainTextResponse(
                                    reply_xml, status_code=200, media_type="text/xml; charset=utf-8"
                                )
            ...
            # 同步处理模式（原有逻辑）
            ai_response = await message_processor.process_message(user_id=from_user, content=content)
        ...
        # 处理图片消息（如果启用多模态）
            ...
            if pic_url:
                ai_response = await message_processor.process_message(
                    user_id=from_user, content="请分析这张图片", image_url=pic_url
                )
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    f"{config.vertex_flow_api_url}/health", timeout=aiohttp.ClientTimeout(total=5)
                ) as response:
                    if response.status != 200:
                        return {"status": "error", "message": f"Vertex Flow API不可用，状态码: {response.status}"}
        except Exception as e:
            return {"status": "error", "message": f"无法连接到Vertex Flow API: {str(e)}"}
...
async def get_async_status():
    ...
    if not ENABLE_ASYNC_PROCESSING:
        return {"enabled": False, "message": "异步处理未启用"}

    try:
        async_proc = await get_async_processor()
        status = async_proc.get_queue_status()

        return {"enabled": True, "status": status, "message": "异步处理器运行正常"}
    except Exception as e:
        return {"enabled": True, "error": str(e), "message": "获取异步处理状态失败"}
